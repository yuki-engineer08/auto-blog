...
description: "個人ブログの記事をQiitaとZennに自動でクロスポストする仕組みをGitHub Actionsで構築した記録。QiitaのREST APIとZennのGitHubリポジトリ連携という全く異なる仕組みの違いや、MDXのJSXコンポーネントが消える問題など、実装で詰まったポイントをまとめる。"
date: "2026-07-14"
tags: ["ci-cd", "deploy", "blog"]
published: true
...
![Next.jsブログをQiitaとZennに自動クロスポストする仕組みのイメージイラスト。git pushから自サイト・Qiita・Zennへ矢印が伸びている](./thumbnail.png)
...
<RelatedPostCard slug="2026-06-25-caregiver-built-cicd-with-terraform" />
...
![QiitaとZennの仕組みの違いを表したイラスト。Qiitaは鍵とAPIトークンでREST APIに直接投稿し、ZennはGitリポジトリに記事ファイルを配置してpushする](./qiita-zenn-diff.png)
...
<RelatedPostCard slug="2026-06-24-caregiver-built-ai-team-with-claude-code" />

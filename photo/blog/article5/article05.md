...
Next.jsを`output: 'export'`で静的エクスポートすると、`/posts/hello`というURLは`/posts/hello.html`または`/posts/hello/index.html`としてファイルが生成される。

CloudFrontはデフォルトでは、`/posts/hello`というリクエストに対してS3の`/posts/hello`というキーを探しに行く。でもそんなキーは存在しない。存在するのは`/posts/hello/index.html`だ。

だから403になる。

```
リクエスト: /posts/hello
CloudFrontが探すS3キー: /posts/hello        → 存在しない → 403

本当に存在するS3キー: /posts/hello/index.html
```

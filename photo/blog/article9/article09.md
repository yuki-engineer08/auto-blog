# Next.jsブログをQiitaとZennに自動クロスポストする仕組みを作った話
...
```
git push (main)
  ├── deploy.yml（常に発火）→ S3 + CloudFront
  ├── qiita-crosspost.yml（content/blog/**/*.mdx 変更時のみ）
  └── zenn-crosspost.yml（同上）
```
...
```
detect-changes.ts    # 変更ファイルを検出
  ↓
convert-mdx.ts       # MDXをプレーンなMarkdownに変換
  ↓
Qiita: post-qiita.ts      → 新規時のみ write-back-id.ts でIDをコミット
Zenn:  push-zenn.ts       → 別リポジトリにpush
```
...
```mdx
<RelatedPostCard slug="2026-06-27-caregiver-built-ai-accident-report-tool" />
```
